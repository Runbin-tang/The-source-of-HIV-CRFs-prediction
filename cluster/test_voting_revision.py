    ...
    for i in range(len(p_label)):
        if len(p_label[i])==2:       # one label
            ...
            if 'refpure' not in vars():
                ...
                refpure=dataprogress.loadData('ref/'+datatype+'pure_ref.txt')
            #if len(p_label[i])>2:
...
def find_crf(data,p_label):
    ...
    if 'refcrf' not in vars():
        ...
        refcrf=dataprogress.loadData('ref/'+datatype+'crf_ref.txt')
...
def voting_pre(data,datatype,stype):   #data is the path of data, datatype is the one of cg and pol
    ...
    name=['model/breknna_model','model/mlknn_model','model/mlaram_model']
    modelname=[]
    for i in name:
...
def judge_unkowstype(data):
    ...
    if 'refcrf' not in vars():
        refcrf=np.load('ref/'+datatype+'crf_ref.txt')#dataprogress.loadData('refcrf.txt')
        
    if 'refpure' not in vars():
        refpure=np.load('ref/'+datatype+'pure_ref.txt')#dataprogress.loadData('refpure.txt')
...
if __name__ == '__main__': 
    ...
    global pool,refcrf,refpure,refnum,te,f,datatype     
